...
class Sticky extends Component {
  ...
  getTopPosition(top) {
    ...
    if (this.props.stickyToPageBottom) {
      return winHeight - this.state.height;
    }
  ...
  updateInitialDimension(options) {
    ...
    const outer = this.outerDom;
    const inner = this.innerDom;
    ...
    // console.log(this.props.stickyToPageBottom, winHeight, height);
    this.setState({
   ...
   */
  update() {
    const { stickyToPageBottom } = this.props;
    if (stickyToPageBottom) {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    return (
      !this.props.shouldFreeze() && shallowCompare(this, nextProps, nextState)
    );
  }
  ...
  render() {
    ...
    if (
      this.props.stickyToPageBottom &&
      this.state.status !== STATUS_RELEASED
    ) {
      innerStyle.position = "fixed";
      innerStyle.top = "0px";
      innerStyle.width = `${this.state.width}px`;
      this.translate(innerStyle, winHeight - this.state.height);
      outerStyle.height = `${this.state.hegiht}px`;
    } else {
    ...
    return (
      <div ref={(n) => { this.outerDom = n; }} className={outerClasses} style={outerStyle}>
        <div ref={(n) => { this.innerDom = n; }} className="sticky-inner-wrapper" style={innerStyle}>
          {this.props.children}
        </div>
      </div>
    );
...
Sticky.propTypes = {
  ...
  stickyToPageBottom: PropTypes.bool,
